...
class PostUpdateViewTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse('boards:edit_post',kwargs={'pk': self.board.pk,
...
class LoginRequiredPostUpdateViewTests(PostUpdateViewTestCase):
    def test_redirection(self):
        ...
        redirected_url = '{login_url}?next={url}'.format(login_url=login_url,
                                                         url= self.url)
...
class UnauthorizedPostUpdateViewTests(PostUpdateViewTestCase):
    def setUp(self):
        ...
        username = 'jane'
        password = '321'
        user = User.objects.create_user(username, 'jane@doe.com', password)
        self.client.login(username=username, password=password)
        self.response = self.client.get(self.url)

    def test_status_code(self):
        """
        A topic should be edited only by the owner. Unauthorized users
        should get a 404 response (Page Not Found).
        """
...
class PostUpdateViewTests(PostUpdateViewTestCase):
    ...
    def test_form_inputs(self):
        """
        The view must containt two inputs, csrf and message textarea.
        """
...
class SuccessfulPostUpdateViewTests(PostUpdateViewTestCase):
    ...
    def test_redirection(self):
        ...
        """
        topic_posts_url = reverse('boards:topic_posts', kwargs={
            'pk': self.board.pk,
            'topic_pk': self.topic.pk
        })
...
class InvalidPostUpdateViewTests(PostUpdateViewTestCase):
    ...
    def test_status_code(self):
        """
        An incalid form submission should return to the same page.
        """
        self.assertEqual(self.response.status_code, 200)
